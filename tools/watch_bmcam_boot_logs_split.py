...
Split-view Tailscale SSH watcher for bmcam001 and bmcam002 RC boot-capture logs.
...
v0.3 changes:
  - Suppresses transient Tailscale/SSH error text from the log panes.
...
Run from Windows PowerShell / VS Code terminal:
...
DEFAULT_TAILSCALE_EXE = r"C:\Program Files\Tailscale\tailscale.exe"
...
def find_tailscale_exe(preferred: str | None) -> str:
    if preferred:
        return preferred
    if os.name == "nt":
        if os.path.exists(DEFAULT_TAILSCALE_EXE):
...
def device_worker(
    ...
    while not stop_event.is_set():
        ...
        cmd = [tailscale_exe, "ssh", f"{device.user}@{device.host}", remote_cmd]
        ...
        try:
            ...
            device.set_status(f"tailscale executable not found: {tailscale_exe}")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Split-view Tailscale SSH watcher for bmcam001/bmcam002 RC boot-capture cron logs."
    )
    ...
    parser.add_argument("--tailscale-exe", default=None, help=r'Tailscale executable path. Default tries "C:\Program Files\Tailscale\tailscale.exe" then PATH.')
...
def main(argv: list[str]) -> int:
    ...
    tailscale_exe = find_tailscale_exe(args.tailscale_exe)
    devices: list[DeviceState] = []
    for host in hosts[:2]:
        label = host.split(".")[0]
    ...
    for device in devices:
        t = threading.Thread(
            ...
            args=(device, tailscale_exe, args.log_glob, args.initial_lines, args.remote_poll_sec, args.retry_delay_sec, stop_event, args.plain),
            daemon=True,
        )
        threads.append(t)
        t.start()
